 ...
 * Return: int
 */
static int spi_disable_irq(struct nfc_dev *dev)
{
	unsigned long flags;
 ...
 * Return: int
 */
static int spi_enable_irq(struct nfc_dev *dev)
{
	unsigned long flags;
 ...
 */
static int get_spi_command_length(int actual_length)
{
...
static int nfc_spi_read(struct nfc_dev *nfc_dev, char *buf, size_t count, int timeout)
{
	int ret;
	struct spi_dev *spi_dev = &nfc_dev->spi_dev;
...
static int nfc_spi_write(struct nfc_dev *nfc_dev, const char *buf, size_t count,
	       int max_retry_cnt)
{
	int ret = -EINVAL;
	int retry_cnt;
...
static ssize_t nfc_spi_dev_read(struct file *filp, char __user *buf, size_t count,
			 loff_t *offset)
{
	int ret;
...
static ssize_t nfc_spi_dev_write(struct file *filp, const char __user *buf,
